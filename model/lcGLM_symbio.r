...
divTreePath <- 'output/symbio_phylo/symportal_plus_voolstraDB_aligned_FastTreeGTR.tree' #symportal div tree
hostTreePath <- 'output/host_phylo/combined_trees.newick' #set of Bayesian draws of host species phylogeny
mapfilePath <- 'raw_data/occurence/GCMP_symbiodinium_map3.txt' #mapping file
...
for (i in 1:NTrees) {
    standat[[i]] <- list(NSamples                       = NSamples,
                         ...
                         modelMatRInv                   = ginv(qr.R(qr(cbind(modelMat, hostAncestorsExpanded[[i]] %*% diag(hostTreeDetails[[i]]$edgeLengthsRel)))) / nrow(modelMat)),
                         NSumTo0                        = NSumTo0,
                         ...
                         microbeAncestorsRInvT          = t(ginv(qr.R(qr(cbind(1, rbind(0, microbeAncestors %*% diag(microbeTreeDetails$edgeLengthsRel))))) / nrow(microbeAncestors))),
                         microbeAncestorsRInvT2         = t(ginv(qr.R(qr(microbeAncestors)) / nrow(microbeAncestors))),
                         hostAncestors                  = hostAncestors[[i]],
                         hostTipAncestors               = hostAncestors[[i]][1:NHostTips, ],
                         hostAncestorsRInv              = ginv(qr.R(qr(hostAncestors[[i]])) / nrow(hostAncestors[[i]])),
