...
parameters {
    ...
    simplex[2] codivVsCophyMetaVarProps[3]; //two distinct patterns are important in evolution: random drift over time (brownian or ornstein-uhlenbeck models of covariance, contigent on branch lengths), and punctuated equilibrium (one form of which entails rapid evolutionary shifts during divergence events, and no evolution over time in general). this generalization of the model adds discrete evolution that occurs at divergence events by using the same methods as the brownian model, but with each edge length set to 1. Both OU and divergence-based variance compete in the same model to explain the data.
